...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float):
        ...
        # create a vector of shape (seq_len), for position of the word in sentence
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model: int, h: int, dropout: float) -> None:
        ...
        self.d_model = d_model
        self.h = h
        assert self.d_model % h == 0, "d_model isn't divisible by h"
    ...
    def forward(self, q, k, v, mask):
        ...
        x, attention_score = MultiHeadAttention.attention(
...
class ResidualConnection(nn.Module):
    "Add and Norm in Transformer Encoder"
...
def build_transformer(
    ...
    # create the encoder ans decoder
    encoder = Encoder(nn.ModuleList(encoder_blocks))
    decoder = Decoder(nn.ModuleList(decoder_blocks))

    # create the projection (Linear layer at the end) layer
